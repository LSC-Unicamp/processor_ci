...
def generate_instance(code: str, mapping: dict, instance_name="u_instancia"):
    ...
    if not (is_vhdl or is_verilog):
        raise ValueError("Não foi possível identificar se é Verilog/SystemVerilog ou VHDL.")
    ...
    if is_verilog:
        ...
        if param_block:
            param_lines = re.findall(r'parameter\s+(\w+)\s*=\s*([^,\n)]+)', param_block.group(1))
    ...
    # --------------------------
        ...
        generic_match = re.search(r'generic\s*\((.*?)\)\s*;', code, re.DOTALL | re.IGNORECASE)
        if generic_match:
            generics = re.findall(r'(\w+)\s*:\s*.*?:=\s*([^;]+);?', generic_match.group(1))
            for name, val in generics:
                params.append((name.strip(), val.strip()))

        # Extrair portas
        port_block_match = re.search(r'port\s*\((.*?)\);\s*end', code, re.DOTALL | re.IGNORECASE)
