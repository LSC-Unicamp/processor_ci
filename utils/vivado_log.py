...
def parse_vivado_utilization(filename, core, board):
    ...
    with open(filename, "r") as f:
        ...
        if luts_match:
            ...
            print("Warning: It was not possible to find total and available LUTs")
...
def parse_vivado_timing(filename):
    with open(filename, "r") as f:
        content = f.read()
        current_clock = re.search(r"sys_clk_pin\s+\S+\s+\S+\s+(\d+\.\d+)", content)
...
if __name__ == "__main__":
    ...
    timing_results = parse_vivado_timing(path.join(args.directory, args.timing_file))
    ...
    out = write_results(
        results, path.join(args.output_folder, f"synth_{args.core}_{args.board}.json")
    )
