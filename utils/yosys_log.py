...
def parse_nextpnr_log(filename, core, board):
    ...
    with open(filename, "r") as f:
        ...
        freq_match = re.findall(r"Max frequency for clock .+?: (\d+\.\d+) MHz", content)
        if freq_match:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--directory", "-d", default=".", help="Directory where the log file is"
    )
    ...
    output = path.join(args.output_folder, f"synth_{args.core}_{args.board}.json")
