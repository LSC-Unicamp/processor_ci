...
def parse_vhdl_ports(text):
    ports = []
    port_block = re.search(r"port\s*\((.*?)\);\s*end", text, re.DOTALL | re.IGNORECASE)
    if not port_block:
        return ports
    ...
    for line in port_block.group(1).splitlines():
        ...
        if match:
            ...
            width = match.group(4)
            if width:
                range_match = re.match(r"(\d+)\s+downto\s+(\d+)", width, re.IGNORECASE)
                if range_match:
...
def generate_instance(module_name, ports, params=[]):
    ...
    for direction, width, name in ports:
        ...
        instance += f"    .{name:<20} ({name}),{' '*(30 - len(name))}// {comment}\n"
    instance = instance.rstrip(",\n") + "\n);"
    return instance
