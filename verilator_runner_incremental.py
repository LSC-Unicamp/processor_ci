...
from verilator_runner import _order_sv_files
...
def _find_file_declaring_package(repo_root: str, package_name: str, module_graph: Dict[str, Dict]) -> List[str]:
    ...
    if not module_graph:
        print_yellow(f"[INCREMENTAL-DEBUG] No module_graph, falling back to file search for package: {package_name}")
    ...
    if found_files:
        if len(found_files) > 1:
            print_green(f"[INCREMENTAL-DEBUG] Found {len(found_files)} files for package '{package_name}': {', '.join(found_files)}")
        else:
            print_green(f"[INCREMENTAL-DEBUG] Found package '{package_name}': {found_files[0]}")
        return found_files
    
    print_yellow(f"[INCREMENTAL-DEBUG] Package '{package_name}' not in module_graph, trying file search...")
    results = _find_file_by_search(repo_root, package_name, "package")
    if results:
        if len(results) > 1:
            print_green(f"[INCREMENTAL-DEBUG] Found {len(results)} files for package '{package_name}' via file search")
        else:
            print_green(f"[INCREMENTAL-DEBUG] Found package '{package_name}' via file search: {results[0]}")
    else:
        print_red(f"[INCREMENTAL-DEBUG] Could not find package '{package_name}' anywhere")
    return results
...
def _detect_language_for_files(repo_root: str, files: List[str], module_graph: Dict[str, Dict] = None, include_dirs: Set[str] = None) -> str:
    ...
    print_yellow(f"[LANG-DEBUG] Detecting language for {len(files)} files + {len(files_to_check) - len(files)} dependencies/includes")
    print_yellow(f"[LANG-DEBUG] Checking: {[os.path.basename(f) for f in list(files_to_check)[:5]]}...")
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                if sv_patterns['always_ff'].search(content):
                    has_always_ff = True
                    has_sv_features = True
                    print_yellow(f"[LANG-DEBUG] Found always_ff/comb/latch in {os.path.basename(file_path)}")
    ...
    if has_sv_features:
        print_yellow("[LANG] Detected SystemVerilog features (always_ff/comb, logic, etc.), using: 1800-2017")
        return "1800-2017"
    ...
    if _has_sv_keyword_as_identifier(repo_root, list(files_to_check)):
        print_yellow("[LANG] Detected SystemVerilog keywords used as identifiers in plain Verilog, forcing: 1364-2005")
        return "1364-2005"
...
def compile_incremental(
    ...
    for iteration in range(max_iterations):
        ...
        print_blue(f"[INCREMENTAL-DEBUG] Command: {' '.join(cmd)}")
