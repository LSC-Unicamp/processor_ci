    ...
    stages {
        stage('Git Clone') {
            steps {
                sh 'rm -rf *.xml'
                sh 'rm -rf lizard'
                sh 'git clone --recursive --depth=1 https://github.com/cornell-brg/lizard lizard'
            }
        }
        ...
        stage('Simulation') {
            steps {
                dir("lizard") {
                    sh "iverilog -o simulation.out -g2005                  -s   asic/designs/GcdUnit-demo.v "
                }
            }
        }
                    ...
                    stages {
                        stage('Synthesis and PnR') {
                            steps {
                                ...
                                sh 'python3 /eda/processor_ci/main.py -c /eda/processor_ci/config \
                                            -p lizard -b digilent_arty_a7_100t'
                                }
                            }
                        }
                        stage('Flash digilent_arty_a7_100t') {
                            steps {
                                ...
                                sh 'python3 /eda/processor_ci/main.py -c /eda/processor_ci/config \
                                            -p lizard -b digilent_arty_a7_100t -l'
                                }
                            }
                        }
