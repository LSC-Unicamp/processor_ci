...
pipeline {
    ...
    stages {
        ...
        stage('Simulation') {
            steps {
                dir("core") {
                    sh "iverilog -o simulation.out -g2005                  -s   lib/LibFPGA/AutoReset.v lib/LibFPGA/BinaryCounter.v lib/LibFPGA/LCDControler.v lib/LibFPGA/LFSR.v lib/LibFPGA/MyPLL.v lib/LibFPGA/VGAGenerator.v "
                }
            }
        }
                    ...
                    stages {
                        stage('Synthesis and PnR') {
                            steps {
                                ...
                                sh 'python3 /eda/processor_ci/main.py -c /eda/processor_ci/config \
                                            -p core -b digilent_arty_a7_100t'
                                }
                            }
                        }
                        stage('Flash digilent_arty_a7_100t') {
                            steps {
                                ...
                                sh 'python3 /eda/processor_ci/main.py -c /eda/processor_ci/config \
                                            -p core -b digilent_arty_a7_100t -l'
                                }
                            }
                        }
